    ...
    public function export(Request $request)
{
    ...
    $sheet = $spreadsheet->getActiveSheet();
        // Thiết lập kích thước trang A4 ngang
    ...
    $sheet->getPageSetup()->setPaperSize(\PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::PAPERSIZE_A4);
        ...
        // Thiết lập header với tiêu đề cho bảng lương tháng
        $start = Carbon::createFromFormat('Y-m', $request->time)->startOfMonth();
        $end = Carbon::createFromFormat('Y-m', $request->time)->endOfMonth();
        $sheet->setCellValue('A3', 'Bảng lương tháng ' . $request->time);
        $sheet->mergeCells('A3:V3'); // Gộp các ô cho tiêu đề
        $sheet->getStyle('A3')->getFont()->setBold(true)->setSize(20); // Tăng kích thước và in đậm tiêu đề
        $sheet->getStyle('A3')->getAlignment()->setHorizontal('center');

        // Cách ra một dòng trống giữa tiêu đề và nội dung
        $sheet->setCellValue('A5', ''); // Dòng trống
    ...
    $sheet->getStyle('A6:W8')->getAlignment()
        ...
        ->setVertical('center');
    // Gộp các ô cho các tiêu đề chính (ngoại trừ "Phụ cấp" đã gộp)
    ...
    // Duyệt qua dữ liệu và điền vào các ô tương ứng
...
if($tax_salary < 0) {$tax_salary = 0;}
    ...
    // Thêm footer với thông tin người tạo và kế toán trưởng
    ...
    // Tạo một đối tượng Writer cho định dạng Xlsx
    $writer = new Xlsx($spreadsheet);
    $filePath = storage_path('app/user_info.xlsx');
    $writer->save($filePath);

    // Tạo response để tải file về máy
